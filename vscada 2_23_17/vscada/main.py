...
try:
	_psu_control = magna_control.PowerSupplyControl('/dev/ttyUSB0', 19200) #Original Code
	#_psu_control = magna_control.PowerSupplyControl('/dev/ttyS0', 19200)
	#_psu_control = magna_control.PowerSupplyControl()
except:
...
class Window(QtGui.QMainWindow, ui_main_window.Ui_MainWindow):
	...
	def guiUpdate(self):
		...
		#if(_motorTemp >= 60):
		 #self.lcdNumberMTemp. #Set Color to Yellow
		#if(_motorTemp >= 80 ):
		 #self.lcdNumberMTemp #Set Color to Red
		if(_motorTemp <= 60):
		 self.lcdNumberMTemp.setStyleSheet(_fromUtf8("color:red;")) #Set Color to Green
